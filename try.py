...
subpage_form_access=True  #debug
subpages={} 
...
def main_form():
    ...
    for name in subpages.keys():
        create_subpage_labels(scroll_frame, name, content, canvas)
    ...
    tk.Button(btn_container, 
              ...
              bd=2, 
              padx=20,
              command=lambda: messagebox.showinfo("Save", "Data saved! (Database integration pending)")).pack(side="right", padx=6)
...
def verify_close(main_app):
    if messagebox.askyesno("Quit?", "Are you sure you want to quit? Any unsaved changes cannot be recovered."):
        main_app.destroy()
...
def show_subpage(content_frame, name):
    ...
    tk.Button(header, 
              ...
              relief="raised", 
              bd=2,
              command=lambda: add_account_dialog(content_frame, name)).pack(side="right")
    
    canvas, scrollable_frame=setup_scrollable_frame(content_frame, bg="white")
    
    accounts=subpages[name]
    if not accounts:
...
def refresh_subpage_list(parent, content_frame, canvas):
    ...
    for name in subpages.keys():
        create_subpage_labels(parent, name, content_frame, canvas)
...
def add_subpage(scroll_frame, canvas, content_frame):
    ...
    def save():
        ...
        if not name or name in subpages or name=="Home":
            messagebox.showwarning("Invalid", "Name already exists.")
            return
...
def delete_subpage(scroll_frame, content_frame, canvas):
    ...
    def save():
        global selected_subpage
        if selected_subpage in subpages:
            del subpages[selected_subpage]
        selected_subpage="Home"
        dialog.destroy()
        refresh_subpage_list(scroll_frame, content_frame, canvas)
...
def rename_subpage(scroll_frame, content_frame, canvas):
    ...
    def save():
        ...
        if not new_name or new_name in subpages or new_name=="Home":
            messagebox.showwarning("Invalid", "Name already exists.")
            return

        subpages[new_name]=subpages.pop(selected_subpage)
...
def delete_account(account, subpage_name, content_frame):
    if messagebox.askyesno("Delete Account?", f"Delete '{account.get('name', 'this account')}'?"):
        subpages[subpage_name].remove(account)
...
def add_account_dialog(content_frame, subpage_name):
    ...
    def save():
        ...
        if not data['name'] or not data['username'] or not data['password']:
            ...
            return
        subpages[subpage_name].append(data)
        dialog.destroy()
        show_subpage(content_frame, subpage_name)
...
def create_dialog_buttons(dialog, save_cmd, save_text="Save", save_color="#0078d4", save_fg="white"):
    ...
    tk.Button(btn_frame, text=save_text, bg=save_color, fg=save_fg, font=(FONT, 10), width=10, 
              relief="raised", bd=2, command=save_cmd).pack(side="left", padx=7, anchor="se")
    tk.Button(btn_frame, text="Cancel", font=(FONT, 10), width=10, bg="#e0e0e0",
              relief="raised", bd=2, command=dialog.destroy).pack(side="left", padx=7, anchor="se")
